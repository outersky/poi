 ...
 * @author Yegor Kozlov
 */
public final class Averageif2 implements FreeRefFunction {
    public static final FreeRefFunction instance = new Averageif2();
    ...
    public ValueEval evaluate(ValueEval[] args, OperationEvaluationContext ec) {
		...
		try {
            AreaEval sumRange = convertRangeArg(args[2]);

            // collect pairs of ranges and criteria
            AreaEval[] ae = new AreaEval[1];
            I_MatchPredicate[] mp = new I_MatchPredicate[1];

            ae[0] = convertRangeArg(args[0]);
            mp[0] = Countif.createCriteriaPredicate(args[1], ec.getRowIndex(), ec.getColumnIndex());
        ...
        for (int r = 0; r < height; r++) {
            for (int c = 0; c < width; c++) {
                ...
                if(matches) { // sum only if all of the corresponding criteria specified are true for that cell.
                    Pair<Double,Integer> p = accumulate(aeSum, r, c);
                    result += p.a;
                    count += p.b;
                }
            }
	...
	private static Pair<Double,Integer> accumulate(AreaEval aeSum, int relRowIndex,
			int relColIndex) {
